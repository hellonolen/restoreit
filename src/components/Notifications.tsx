...
export function Notifications({ notifications }: NotificationsProps) {
    ...
    return (
        <div className="fixed bottom-6 right-6 z-50 space-y-2 pointer-events-none" aria-live="polite" aria-atomic="false">
            {notifications.map(n => (
                <div key={n.id} className="flex items-center gap-3 px-4 py-3 rounded-xl border shadow-xl text-sm max-w-sm animate-in slide-in-from-right-4 duration-300 bg-[var(--color-background-elevated)] border-[var(--color-border)] text-[var(--color-text-secondary)]" role="alert">
                    {icons[n.type]}
