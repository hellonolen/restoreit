                    ...
                    <div className="relative z-10">
                        ...
                        <div className="space-y-8 max-w-lg">
                            ...
                            <p className="text-lg text-[var(--color-text-tertiary)] leading-relaxed font-medium max-w-sm">
                                See what the scan finds before you pay.
                            </p>
                            ...
                            <div className="space-y-4 pt-4">
                                {[
                                    ...
                                    'Sector-by-sector cloud scanning',
                                ].map(feature => (
                                    <div key={feature} className="flex items-center gap-3 text-sm text-[var(--color-text-tertiary)]">
                    ...
                    <div className="w-full max-w-md space-y-10">
                        ...
                        <form onSubmit={handleSubmit} className="space-y-6">
                            ...
                            <div className="space-y-5">
                                <div className="space-y-2">
                                    ...
                                    <div className="relative group">
                                        ...
                                        <Mail size={16} className="absolute right-6 top-1/2 -translate-y-1/2 text-[var(--color-placeholder)] group-focus-within:text-[var(--color-text-tertiary)] transition-colors" />
                                    </div>
                                </div>

                                <div className="space-y-2">
                                    <div className="flex items-center justify-between ml-1">
                                        <label className="text-xs font-bold text-[var(--color-text-tertiary)] uppercase tracking-wider">Password</label>
                                        <Link href="/support" className="text-xs font-bold text-[var(--color-accent)] hover:opacity-80 transition-all">Forgot password?</Link>
                                    </div>
                                    <div className="relative group">
                                        <input type={showPassword ? 'text' : 'password'} placeholder="••••••••" value={password} onChange={e => setPassword(e.target.value)}
                                            className="w-full bg-[var(--color-input-bg)] border border-[var(--color-border)] rounded-2xl px-6 py-5 text-sm text-[var(--color-foreground)] placeholder-[var(--color-placeholder)] outline-none focus:border-[var(--color-accent)]/50 focus:bg-[var(--color-accent)]/[0.02] transition-all font-medium" />
                                        <button type="button" onClick={() => setShowPassword(p => !p)} className="absolute right-6 top-1/2 -translate-y-1/2 text-[var(--color-placeholder)] hover:text-[var(--color-text-secondary)] transition-colors">
                            ...
                            <button type="submit" disabled={loading}
                                ...
                                {loading ? (
                                    <><div className="w-5 h-5 border-2 border-[var(--color-border)] border-t-[var(--color-foreground)] rounded-full animate-spin" /> Signing in...</>
                                ) : (
