...
const faqs = [
    ...
    { q: 'Can I scan a formatted drive?', a: 'Yes. Formatting a drive deletes the file table but usually does not zero out the data sectors. restoreit\'s Deep Scan reads every sector and detects files from raw binary patterns, independent of the file table. Results vary depending on the state of the drive.' },
    { q: 'What if no files are found?', a: 'You are not charged. Our pricing model gates payment behind file detection. If the scan returns zero files, the session closes at no cost.' },
    { q: 'How long does my data stay in the restoreit cloud?', a: 'Standard tier: your restored files are retained in restoreit Cloud for 48 hours, after which they are automatically purged. Pro tier: your data is retained in encrypted restoreit Cloud for 7 days. Protection Plan subscribers receive extended 30-day retention.' },
        ...
        <div className="min-h-screen bg-[var(--color-background)] text-[var(--color-foreground)] font-sans flex flex-col transition-colors duration-300">
            ...
            <main className="flex-1 pt-28 max-w-3xl mx-auto px-6 py-16 space-y-16 w-full">
                ...
                <div>
                    ...
                    <p className="text-[var(--color-text-secondary)] leading-relaxed">Everything you need to understand how restoreit works and resolve common issues.</p>
                </div>
                ...
                <section>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                        {[
                            ...
                            <div key={os} className="p-4 rounded-xl border border-[var(--color-border)] bg-[var(--color-card)]">
                                <div className="font-semibold text-sm mb-1">{os}</div>
                                <div className="text-[var(--color-accent)] text-xs font-mono mb-1">{req}</div>
                                <div className="text-[var(--color-text-dim)] text-[11px]">{note}</div>
                            </div>
                        ))}
                    </div>
                    <div className="mt-3 text-xs text-[var(--color-text-dim)] flex items-center gap-2">
                ...
                <section>
                    ...
                    <div className="p-6 rounded-xl border border-[var(--color-border)] bg-[var(--color-card)] space-y-4 text-sm text-[var(--color-text-secondary)] leading-relaxed">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-2">
                            {[
                                ...
                                <div key={label} className="flex items-center gap-2 p-3 rounded-xl bg-[var(--color-card)] border border-[var(--color-border-subtle)] text-xs text-[var(--color-accent)]">
                                    {icon}<span className="text-[var(--color-text-secondary)]">{label}</span>
                                </div>
                            ))}
                        </div>
                    </div>
                ...
                <section>
                    ...
                    <div className="space-y-3">
                        {[
                            ...
                            <details key={problem} className="group p-4 rounded-xl border border-[var(--color-border)] bg-[var(--color-card)] cursor-pointer">
                                <summary className="flex items-center justify-between text-sm font-medium list-none">
                                    {problem}
                                    <ChevronRight size={16} className="text-[var(--color-text-dim)] group-open:rotate-90 transition-transform" />
                                </summary>
                                <p className="mt-3 text-sm text-[var(--color-text-tertiary)] leading-relaxed">{fix}</p>
                            </details>
                        ))}
                    </div>
                </section>
                ...
                <section>
                    ...
                    <div className="space-y-3">
                        {faqs.map(({ q, a }) => (
                            <details key={q} className="group p-5 rounded-xl border border-[var(--color-border)] bg-[var(--color-card)] cursor-pointer">
                                <summary className="flex items-center justify-between text-sm font-medium list-none pr-2">
                                    {q}
                                    <ChevronRight size={16} className="text-[var(--color-text-dim)] shrink-0 group-open:rotate-90 transition-transform" />
                                </summary>
                                <p className="mt-3 text-sm text-[var(--color-text-tertiary)] leading-relaxed">{a}</p>
                            </details>
                        ))}
                    </div>
                </section>

                <div className="text-center text-xs text-[var(--color-disabled-text)] pb-8">
                    Still stuck? Email us at <a href="mailto:support@restoreit.app" className="text-[var(--color-accent)] hover:underline">support@restoreit.app</a>. Pro subscribers receive priority response within 4 hours.
                </div>
            </main>
